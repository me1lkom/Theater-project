...
export default function PlayAvailableSeats({ sessionId }) {
    ...
    // При смене sessionId - полный сброс
    useEffect(() => {
        if (!sessionId) return;
    ...
    // Когда данные готовы - инициализируем
    useEffect(() => {
        if (!seats || !availableSeats) return;
        if (!svgRef.current) return;
    ...
    const handleBooking = async () => {
        ...
        // selectedSeats — это уже массив ID (из вашего кода)
    ...
    return (
        <div className="seatsInfo">
            ...
            <div style={{ marginTop: 20, padding: 15, background: '#2a2a2a', borderRadius: 10, color: 'white' }}>
                <div>Выбрано мест: <strong style={{ color: '#f1c40f', fontSize: 24 }}>{selectedSeats.length}</strong></div>
                <button
                    onClick={handleBooking}
                    style={{ marginTop: 10, padding: '8px 20px', background: '#e74c3c', border: 'none', borderRadius: 6, color: 'white', cursor: 'pointer' }}
