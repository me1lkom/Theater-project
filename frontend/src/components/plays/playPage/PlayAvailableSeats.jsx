...
export default function PlayAvailableSeats({ sessionId }) {
    ...
    const svgRef = useRef(null); // 👈 используем ref вместо getElementById
    const initialized = useRef(false);

    // Инициализация карты после того как SVG отрисовался
    useEffect(() => {
        ...
        // Фильтруем только квадратики мест (исключаем сцену и декорации)
        const seatRects = Array.from(rectangles).filter(rect => {
            const y = parseFloat(rect.getAttribute('y'));
            // Места имеют y от 154 до 914
            return y >= 154 && y <= 914;
        });
        ...
        seatRects.forEach((rect, index) => {
            ...
            // Добавляем номер места (если ещё нет)
            ...
            rect.onclick = (e) => {
                ...
                if (isSelected) {
                    ...
                    rect.setAttribute('fill', seatData.sector_name === 'Партер' ? '#2ecc71' :
                        seatData.sector_name === 'Амфитеатр' ? '#3498db' : '#e67e22');
    ...
    }, [seats, availableSeats]); // Зависимости — когда данные готовы, инициализируем

    // Сброс инициализации при смене sessionId
    useEffect(() => {
    ...
    return (
        <div className="seatsInfo">
            ...
            <div style={{ marginTop: 20, padding: 15, background: '#2a2a2a', borderRadius: 10, color: 'white' }}>
                ...
                <button
                    onClick={() => {
                        const selected = seats.filter(s => selectedSeats.includes(s.seat_id));
                        console.log('Бронирование:', selected);
                        alert(`Забронировано ${selected.length} мест`);
                    }}
